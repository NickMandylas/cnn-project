...
const HistoricalUploadForm: React.FC<HistoricalUploadFormProps> = ({ id }) => {
  ...
  const [createCheck, { loading: loadingCreate }] = useCreateCheckMutation();
      ...
      >
        <Formik
          ...
          onSubmit={async (input) => {
            ...
            if (response.data?.createCheck.check?.id) {
              router.push(
                `/dashboard/patient/check/${response.data?.createCheck.check?.id}`
              );
            }
          }}
        >
